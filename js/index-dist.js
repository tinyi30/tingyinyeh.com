...
angular.module('tingyinyeh', ['matchmedia-ng']).factory('getProjects', function ($http) {
    return function () {
        var cache = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;

        return $http.get('../projects.json', { cache: cache }).then(function (res) {
            return res.data;
        });
    };
    ...
    var queryTablet = 'only screen and (max-width: 992px)';
    ...
    this.getSrcset = function (cover, imagesPath) {
        return '' + imagesPath + cover + '.jpg 1x, ' + imagesPath + cover + '@2x.jpg 2x';
    };

    this.updateDisplayProjects = function () {
        if (!_this.projects) {
            return;
        }
        if (matchmedia.is(queryTablet)) {
            _this.projectsTop = _this.projects.slice(0, 2);
            _this.projectsBottom = _this.projects.slice(2);
        } else {
            _this.projectsTop = _this.projects.slice(0, 4);
            _this.projectsBottom = _this.projects.slice(4);
        }
    };

    getProjects().then(function (projects) {
        _this.projects = projects.data;
        _this.imagesPath = projects.imagesPath;

        _this.updateDisplayProjects();
    });

    matchmedia.on(queryTablet, function () {
        _this.updateDisplayProjects();
    });
});